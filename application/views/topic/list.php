          ...
          <div class="card">
            <div class="card-header">
              <button type="button" class="btn btn-flat btn-success btn_add_new float-right" data-toggle="modal"
                data-target="#new_topic"><i class="fa fa-plus"></i> Create</button>
            </div>
            <!-- /.card-header -->
            <?= form_open(base_url('topic/edit_topic'), [
              'name' => 'form_topic_update',
              'class' => 'form-horizontal',
              'id' => 'form_topic_update',
              ...
              <table id="example2" class="table table-bordered table-hover">
                ...
                <tbody>
                  ...
                  foreach ($topic_list as $topic) {
                    ...
                    <tr>
                      <td><?php echo $sn; ?></td>
                      <td><span class="value"><?php echo $topic->course_name; ?></span><span class="input hidden">
                          <select name="group[<?php echo $topic->id; ?>][course_id]" class="form-control" disabled>
                            <option>Select Course</option>
                            <?php foreach (db_get_all_data('courses') as $row) { ?>
                              <option <?php if($topic->course_id ==$row->id){ echo 'selected';}?> value="<?php echo $row->id; ?>"><?php echo $row->name; ?></option>
                            <?php } ?>
                          </select>
                        </span></td>
                      <td><span class="value"><?= $topic->name; ?></span><span class="input hidden"><input type="text"
                            class="form-control" name="group[<?php echo $topic->id; ?>][name]"
                            value="<?php echo $topic->name; ?>" disabled="disabled"></span></td>
              ...
              <div class="col-md-12 mt-10p">
                <div class="row">
                  <div class="col-xs-6 p-0">
                    <button type="submit" class="btn  btn-primary update_topic" id="btn_edit">Edit
                    </button>
...
<!-- /.content-wrapper -->
<div id="new_topic" class="modal fade new-july-design" class="dialog">
  ...
  <div class="modal-dialog modal-md">
    ...
    <div class="modal-content">
      ...
      <div class="modal-body new-july-design">
        <?= form_open(base_url('topic/save_topic'), [
          'name' => 'form_topic_add',
          'class' => 'form-horizontal',
          'id' => 'form_topic_add',
          'enctype' => 'multipart/form-data',
        ...
        <div class="form-group group-name ">
          <label for="name" class="col-sm-4 control-label"> Chapter<i class="required">*</i>
          </label>
          <div class="col-sm-8">
...
<script>
  $(document).ready(function () {
    ...
    $('.update_topic').click(function () {
      ...
      var form_topic_update = $('#form_topic_update');
      var data_post = form_topic_update.serializeArray();
      ...
      $.ajax({
        url: form_topic_update.attr('action'),
        type: 'POST',
        dataType: 'json',
        data: data_post,
      })
    ...
    $('.btn_save').click(function () {
      var form_topic_add = $('#form_topic_add');
      var data_post = form_topic_add.serializeArray();
      $.ajax({
        url: form_topic_add.attr('action'),
        type: 'POST',
        dataType: 'json',
        data: data_post,
      })
