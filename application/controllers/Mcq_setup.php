<?php
class Mcq_setup  extends CI_Controller
{
    ...
    {
        $data['mcq_list']=$this->mcq_setup_model->get_mcq_question_list();
    ...
    {
        $data['mcq_detail'] =$this->mcq_setup_model->get_mcq_detail($id);
        $data['mcq_exam_detail'] = $this->mcq_setup_model->get_mcq_exam_detail($id);
        $this->load->view('include/header');
    ...
    public function edit_mcq($id){
        ...
        if ($this->form_validation->run() == TRUE) {
			...
			$questions = [];
			foreach ($chapter_data as $key => $data) {
				$this->mcq_setup_model->update_mcq_exam_detail($id, $key, $data['no_of_questions']);
				$questions[] = $data['no_of_questions'];
			}
	...
	{
		$results = $this->mcq_setup_model->get_chapter_details($course_id);
		ob_start(); ?>
